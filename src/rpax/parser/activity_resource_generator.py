...
class ActivityResourceGenerator:
    ...
    def generate_activity_resource(
        self,
        activity: EnhancedActivityNode,
        workflow_name: str,
        project_slug: str,
        all_activities: list[EnhancedActivityNode],
        package_resolver: ActivityPackageResolver,
    ) -> dict[str, Any]:
        ...
        container_info = self.container_parser.parse_container_info(
        ...
        package_uri = None
        if package_source:
            package_uri = self.uri_resolver.generate_uri(
                "projects", project_slug, package_source.package_name, "packages"
            )

        # Find child activities
        child_activities = [
            child.node_id
            for child in all_activities
            if child.parent_id == activity.node_id
        ]
        ...
        resource = {
            ...
            "package_source": {
                ...
                "package_version": (
                    package_source.package_version if package_source else None
                ),
                "classification": (
                    package_source.classification if package_source else None
                ),
            ...
            "relationships": {
                ...
                "invokes_workflow": (
                    activity.invocation_target
                    if hasattr(activity, "invocation_target")
                    and activity.invocation_target
                    else None
                ),
    ...
    def generate_activity_collection(
        self,
        activities: list[EnhancedActivityNode],
        project_slug: str,
        workflow_name: str,
        package_resolver: ActivityPackageResolver,
    ) -> dict[str, Any]:
        ...
        container_activities = sum(
            1 for a in activities if a.tag in ContainerInfoParser.CONTAINER_TYPES
        )
        ...
        for activity in activities:
            if (
                activity.depth == 1
                and activity.tag in ContainerInfoParser.CONTAINER_TYPES
            ):
    ...
    def generate_activities_by_package(
        self,
        activities: list[EnhancedActivityNode],
        project_slug: str,
        package_name: str,
        ...
        return {
            ...
            "relationships": {
                "package_uri": self.uri_resolver.generate_uri(
                    "projects", project_slug, package_name, "packages"
                ),
                "project_uri": self.uri_builder.project(project_slug),
            },
        }
    ...
    def save_activity_resource(
        self, resource: dict[str, Any], workflow_name: str, activity_node_id: str
    ) -> Path:
