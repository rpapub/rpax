...
class EnhancedWorkflowDiscovery(XamlDiscovery):
    ...
    def discover_workflows(self) -> WorkflowIndex:
        ...
        failed_parses = (
            xaml_index.failed_parses
        )  # Coded workflow parsing failures would be logged but not counted here
    ...
    def _discover_coded_workflows(self) -> list[Workflow]:
        ...
        for coded_file in coded_files:
            try:
                ...
                coded_info = self.coded_parser.parse_coded_workflow(
                    coded_file, relative_path_str
                )
...
def create_workflow_discovery(
    project_root: Path,
    exclude_patterns: list[str] = None,
    include_coded_workflows: bool = True,
) -> XamlDiscovery:
