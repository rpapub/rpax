...
class PseudocodeArtifact(BaseModel):
    ...
    format_version: str = Field(
        default="gist-style", description="Pseudocode format version"
    )
    total_lines: int = Field(default=0, description="Total number of pseudocode lines")
    total_activities: int = Field(
        default=0, description="Total number of activities represented"
    )
    entries: list[dict[str, Any]] = Field(
        default_factory=list, description="Legacy pseudocode entries"
    )
    ...
    expanded_pseudocode: list[str] = Field(
        default_factory=list, description="Recursively expanded pseudocode lines"
    )
    expansion_config: dict[str, Any] = Field(
        default_factory=dict, description="Configuration used for expansion"
    )
    generation_config: dict[str, Any] = Field(
        default_factory=dict, description="Configuration used during generation"
    )
