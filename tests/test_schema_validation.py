#!/usr/bin/env python3
"""Test schema validation and versioning system.

This script validates the schema versioning system, backward compatibility,
and migration capabilities for rpax artifacts.
"""
...
from rpax.schema_validator import (
    ArtifactValidator, SchemaRegistry, SchemaVersion, 
    ValidationSeverity, validate_artifact_file
)


def test_schema_registry():
    """Test schema registry loading and version management."""
    print("=== Testing Schema Registry ===")
    
    registry = SchemaRegistry()
    
    # Test schema loading
    activity_schema = registry.get_schema("activity-instances")
    if activity_schema:
        print("OK - Activity instances schema loaded")
        print(f"  - Schema title: {activity_schema.get('title', 'Unknown')}")
    else:
        print("ERROR - Activity instances schema not found")
        return False
    
    # Test version enumeration
    versions = registry.get_available_versions("activity-instances")
    print(f"OK - Available versions: {versions}")
    
    # Test latest version
    latest = registry.get_latest_version("activity-instances") 
    print(f"OK - Latest version: {latest}")
    
    # Test specific version loading
    v1_schema = registry.get_schema("activity-instances", "1.0.0")
    v11_schema = registry.get_schema("activity-instances", "1.1.0")
    
    if v1_schema and v11_schema:
        print("OK - Both v1.0.0 and v1.1.0 schemas loaded")
        
        # Check version differences
        v1_props = set(v1_schema["properties"].keys())
        v11_props = set(v11_schema["properties"].keys())
        new_props = v11_props - v1_props
        
        print(f"  - New properties in v1.1.0: {sorted(new_props)}")
    else:
        print("WARN - Not all schema versions available")
...
def test_schema_version_comparison():
    ...
    print("\n=== Testing Schema Version Comparison ===")
...
def test_real_artifact_validation():
    ...
    print(f"Validated {validated_count}/{len(test_outputs)} real artifacts")
    return validated_count > 0


def main():
    """Run all schema validation tests."""
    print("Testing Schema Validation and Versioning System")
    print("=" * 60)
